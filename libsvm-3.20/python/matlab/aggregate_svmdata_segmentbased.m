...
shapecontext_files = dir(fullfile(features_folder, 'inner shape context', '*_sc.mat')); %12288
concavityprofile_files = dir(fullfile(features_folder, 'concavity descriptors test', '*_conc.mat')); %112
...
%     load(fullfile(features_folder, 'chaincode histograms',chainhist_files(i).name));
    ...
    load(fullfile(features_folder, 'inner shape context',shapecontext_files(i).name));
    load(fullfile(features_folder, 'concavity descriptors test',concavityprofile_files(i).name));
    %     len = length;
    curvature = curvature;
    
    %         all_data(counter,:) = [len, curvature];
    all_data(i,:) = [len, curvature, fsd, inner_sc, ft_vector];
